...
export default function BookResult() {
  ...
  return (
    <section className="flex flex-col justify-center items-center">
      <ul className="py-10 px-6 rounded-lg flex flex-wrap justify-center gap-4">
        {isLoading ? (
          ...
          searchResult && searchResult.map((book: BookType) => (
            <Card key={book.id} className="w-1/4 flex flex-col justify-between items-center shadow-xl hover:drop-shadow-2xl transition-all">
              <CardHeader className="flex justify-center w-[170px] h-auto">
                <img src={book.volumeInfo.imageLinks && book.volumeInfo.imageLinks.thumbnail} className="w-full h-[170px]" alt={book.volumeInfo.title} />
              </CardHeader>
              <CardContent>
